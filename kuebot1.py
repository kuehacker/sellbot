...
def lineBot(op):
    try:
        if op.type == 0:
            print ("[ 0 ] By.KUE ตรวจสอบ")
            return
        if op.type == 5:
            print ("[ 5 ] By.KUE ตรวจสอบบล็อคแล้ว")
            if settings["autoAdd"] == True:
                nadya.blockContact(op.param1)
        if op.type == 13:
            print ("[ 13 ] By.KUE ตรวจสอบ")
            group = nadya.getGroup(op.param1)
            if settings["autoJoin"] == True:
                nadya.acceptGroupInvitation(op.param1)
        if op.type == 24:
            print ("[ 24 ] By.KUE ออกแชทรวมแล้ว")
            if settings["autoLeave"] == True:
                nadya.leaveRoom(op.param1)
        if op.type == 25:
            print ("[ 25 ] By.KUE ตรวจสอบแล้ว")
            msg = op.message
            text = msg.text
            msg_id = msg.id
            receiver = msg.to
                        ...
                        for target in targets:
                            if target in admin:
                                ...
                                try:
                                    ...
                                    nadya.inviteIntoGroup(to,[target])
                                except:
                                    pass
                elif "Ri:" in msg.text:
                    midd = text.replace("Ri:","")
                    nadya.kickoutFromGroup(to,[midd])
                    nadya.findAndAddContactsByMid(midd)
                    nadya.inviteIntoGroup(to,[midd])
#==============FINNISHING PROTECT========================#
                elif msg.text.lower() == 'ตอบรับเปิด':
                        if settings["Wc"] == True:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับมีคนสมาชิกเข้ากลุ่ม   .")
                        else:
                            settings["Wc"] = True
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับมีคนสมาชิกเข้ากลุ่ม   ")
                        ...
                        if settings["Wc"] == False:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับมีคนสมาชิกเข้ากลุ่ม   ")
                        else:
                            settings["Wc"] = False
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับมีคนสมาชิกเข้ากลุ่ม   ")

                elif msg.text.lower() == 'ออกเปิด':
                        if settings["Lv"] == True:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับมีคนสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = True
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับมีคนสมาชิกออกกลุ่ม   ")
                elif msg.text.lower() == 'Q off':
                        if settings["Lv"] == False:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับมีคนสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = False
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับมีคนสมาชิกออกกลุ่ม   ")
...
#==============================================================================#
                    ...
                    nadya.sendMessage(msg.to,"Autorespon disabled.")
                    ...
                    try:
                        ...
                        if strnum == "off":
                                ...
                                if settings["lang"] == "JP":
                                    nadya.sendText(msg.to,"Invitation refused turned off\nTo turn on please specify the number of people and send")
                                else:
                                    nadya.sendText(msg.to,"关了邀请拒绝。要时开请指定人数发送")
                        else:
                                num =  int(strnum)
                                settings["autoCancel"]["on"] = True
                                if settings["lang"] == "JP":
                                    nadya.sendText(msg.to,strnum + " สมาชิกในกลุ่มจะปฏิเสธคำเชิญโดยอัตโนมัติ")
                                else:
                                    nadya.sendText(msg.to,strnum + "使人以下的小组用自动邀请拒绝")
                    except:
                        if settings["lang"] == "JP":
                                nadya.sendText(msg.to,"Value is wrong")
                        else:
                                nadya.sendText(msg.to,"Bizarre ratings")					
                ...
                if text.lower() == 'help':
                    ...
                    nadya.sendContact(to, "ub9f21d57859973fdf6fe7550a0234cd2")
...
#==============================================================================#
                    ...
                    try:
                        arr = []
                        owner = "ub9f21d57859973fdf6fe7550a0234cd2"
...
#==============================================================================#
                    ...
                    nadya.sendContact(to, "Berhasil menonaktifkan Auto Add")
...
#==============================================================================#
        if op.type == 26:
            print ("[ 26 ] By.KUE ตรวจสอบคำคนอืนสั่งได้แล้ว")
            msg = op.message
            text = msg.text
            msg_id = msg.id
            receiver = msg.to
                ...
                if msg.contentType == 0 and sender not in nadyaMID and msg.toType == 2:
                    if "MENTION" in list(msg.contentMetadata.keys()) != None:
                         if settings['detectMention'] == True:
                             ...
                             balas = ["『 0987590523』\n " + cName + "\n『แทคทำไมหรอ?\nไม่ว่างมีรัยก็ทักแชทไม่งันก็โทรมาเลย』"]
                             ret_ = "" + random.choice(balas)
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        if op.type == 17:
        	...
        	nadya.sendMessage(op.param1, "สวัสดี {}, Welcome to Group {}\nเข้ามาแล้วทำตัวดีๆละ\nอ่ย่าไปเป็นบ้าลบเพื่อนๆออกกลุ่มนะ (｀・ω・´)\nขายบอทป้องกันกลุ่มsiri v10 &เชลบอทแถมฟรีออโต้บล็อคสนใจจิ้มมาครับ".format(str(dan.displayName),str(tgb.name)))
...
# ----------------- NOTIFED MEMBER JOIN GROUP#==============================================================================#
        if op.type == 55:
            print ("[ 55 ] By.KUE ตรวจสอบทั้งหมดแล้ว")
            try:
